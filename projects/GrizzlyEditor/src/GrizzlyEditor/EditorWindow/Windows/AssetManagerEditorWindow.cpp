...
namespace GrizzlyEditor {
	...
	void AssetManagerEditorWindow::render_window()
	{
		...
		std::vector<GrizzlyBear::RelativePath> files = GrizzlyBear::Environment::get_fetcher()->get_things_in_dir(m_CurrentPath.get_path());
		...
		for (GrizzlyBear::RelativePath path : files)
		{
			...
			if (ImGui::BeginDragDropSource())
			{
				...
				ImGui::SetDragDropPayload("AM_PushFile", (m_CurrentPath.get_path() + path.m_FullPath.get_name()).c_str(), std::strlen((m_CurrentPath.get_path() + path.m_FullPath.get_name()).c_str()) + 1);
			...
			if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0) && !ImGui::IsMouseDragging(0))
			{
				...
				else
				{
					if (path.m_FullPath.get_extension() == "gcs")
					{
						...
						{
							m_SceneManager->load_scene(path.m_FullPath.get_name_woe(), m_CurrentPath.get_path() + path.m_FullPath.get_name());
						}
		...
		if (ImGui::BeginPopup("CreateLevel##AssetManager"))
		{
			...
			{
				m_SceneManager->unload_scene(m_SceneManager->get_scene_name());
				m_SceneManager->load_scene(m_InputBuf, m_CurrentPath.get_path() + std::string(m_InputBuf) + std::string(".gcs"));
