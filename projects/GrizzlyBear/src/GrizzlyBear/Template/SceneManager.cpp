...
namespace GrizzlyBear {
	...
	namespace Template {
		...
		void Template::SceneManager::set_custom_camera(Ref<Camera> camera)
		{
		...
		Ref<Scene> SceneManager::load_scene(const Gstring& name, const Gstring& fromFile, bool createIfNotExists)
		{
			...
			{
				if (!FileIO::exists(Environment::get_fetcher()->get_full_asset_path(fromFile)) && createIfNotExists)
				{
		...
		void SceneManager::on_update(Timestep ts)
		{
			...
			if (m_CustomCamera != nullptr)
			{
				...
				m_Renderer2D->render_scene(ts, get_scene(), m_CustomCamera->get_projection(), m_CustomCamera->calc_view());
			}
			else
			{
