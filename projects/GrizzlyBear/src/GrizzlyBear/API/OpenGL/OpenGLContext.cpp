...
namespace GrizzlyBear {
		...
		: m_WindowHandle(windowHandle)
	{
		...
		GRIZZLY_CORE_LOG(LogLevel::Info, "  Vendor: {0}", glGetString(GL_VENDOR));
		GRIZZLY_CORE_LOG(LogLevel::Info, "  Renderer: {0}", glGetString(GL_RENDERER));
		GRIZZLY_CORE_LOG(LogLevel::Info, "  Version: {0}", glGetString(GL_VERSION));
	...
	{
		return std::make_shared<OpenGLRenderBuffer>(width, height, internalFormat);
	}
