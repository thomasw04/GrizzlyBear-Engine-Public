...
namespace GrizzlyBear {
	...
	OpenGLRenderBuffer::OpenGLRenderBuffer(int width, int height, int internalFormat)
		: m_RendererID(0), m_InternalFormat(internalFormat), m_Width(width), m_Height(height)
	{
		...
		{
			GL_VALIDATE(glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, width, height));
		}
	}
		...
		: m_RendererID(0), m_Width(width), m_Height(height), m_RenderBufferID(0)
	{
		...
		GL_VALIDATE(glViewport(0, 0, m_Width, m_Height));
	}
	...
	void OpenGLFrameBuffer::unbind(int resetWidth, int resetHeight) const
	{
		...
		GL_VALIDATE(glViewport(0, 0, resetWidth, resetHeight));
	}
