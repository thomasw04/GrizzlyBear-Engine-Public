...
namespace GrizzlyBear {
	...
	int LuaScriptingInstance::set_position(lua_State* L)
	{
		...
		if (entityIndex != 0)
		{
			CompTransformation* transform = m_Scene->get_component<CompTransformation>(MemoryType::Dynamic, entityIndex);
	...
	int LuaScriptingInstance::set_rotation(lua_State* L)
	{
		...
		if (entityIndex != 0)
		{
			...
			transform->m_Rotation.x = rotation;
		}
	...
	int LuaScriptingInstance::get_rotation(lua_State* L)
	{
		...
		if (entityIndex != 0)
		{
			...
			lua_pushnumber(L, transform->m_Rotation.x);
			return 1;
		}
	...
	int LuaScriptingInstance::set_size(lua_State* L)
	{
		...
		if (entityIndex != 0)
		{
			...
			transform->m_Size.x = size.x;
			transform->m_Size.y = size.y;
		}
	...
	void LuaScriptingInstance::set_framework(const Gstring& filepath)
	{
		...
		int e = luaL_loadfile(m_L, Environment::get_fetcher()->get_asset_path(filepath).c_str());
