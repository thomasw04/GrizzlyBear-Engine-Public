...
namespace GrizzlyBear {
	...
	void Renderer2D::load_quad_shader(const Gstring& filepath)
	{
		...
		if (m_Flags & RendererHint_NoEnvironment)
		{
			...
			{
				path = FilePath(FileIO::get_program_directory().str() + filepath.substr(7));
			}
			else
			{
				path = FilePath(FileIO::get_program_directory().str() + filepath);
			}

			buffer = FileIO::get_file_content(path.str());
		}
		else
		{
			buffer = Environment::get_fetcher()->get_asset_data(filepath);
		}
	...
	void Renderer2D::begin_scene(const SceneProps& props)
	{
		...
		glm::mat4 viewProjectionMatrix = props.PROJECTION * glm::inverse(props.TRANSFORMATION);
		m_AmbientLight = props.AMBIENT_LIGHT;
	...
	void Renderer2D::end_quad_batch()
	{
		...
		m_QuadShader->upload_uniform_float_4("u_AmbientColor", m_AmbientLight.COLOR);
		m_QuadShader->upload_uniform_float_1("u_AmbientIntensity", m_AmbientLight.INTENSITY);
	...
	bool Renderer2D::render_scene(Timestep ts, const Ref<Scene> scene)
	{
		...
		CompTransformation* camTransform = scene->get_component<CompTransformation>(MemoryType::Dynamic, scene->get_camera());

		glm::mat4 camTransformationMatrix = glm::translate(glm::mat4(1.0f), camTransform->m_Position) * glm::rotate(glm::mat4(1.0f), glm::radians(camTransform->m_Rotation.x), glm::vec3(0, 0, 1));

		return render_scene(ts, scene, { camera->m_ProjectionMatrix, camTransformationMatrix, scene->get_ambient_light()});
	}

	bool Renderer2D::render_scene(Timestep ts, const Ref<Scene> scene, const SceneProps& custom)
	{
		if (!scene->is_loaded(false, true)) return false;

		begin_scene(custom);

		for (auto it = scene->create_iterator<CompSprite, CompTransformation>(); it != scene->get_end(); it++)
		{
			auto mesh = it.get<CompSprite>();
			auto transform = it.get<CompTransformation>();

			draw_quad(transform->m_Position, transform->m_Rotation.x, { transform->m_Size.x, transform->m_Size.y }, scene->internal_cvt_quad(ts, it.get_index(), mesh));
		}
